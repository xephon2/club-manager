...
/**
 * RuntimeManager is the managing class and uses the singleton pattern, meaning,
 * that the class can only be instantiated once.
 * 
...
public class RuntimeManager implements ClubObserver {
	...
	/**
	 * Holds a single instance of club, meaning that club can only be
	 * instantiated once.
	 */
	private Club club;
	...
	public final boolean matchPassword(final String username,
		...
		String hashedEstimatedPassword = getClub().getClubMemberByUserName(
				username).getHashedPassword();
	...
	/**
	 * The observer method is calld when new messages are consumed from the
	 * subject.
	 */
	@Override
	 ...
	 */
	public final boolean loginUser(final String userName, final String password) {
