...
public class Main {
	...
	public static void main(final String[] args)
		...
		final Club club = new Club("myclub");
		runtimeManager = new RuntimeManager(club);

		final XMLManager xman = new XMLManager(runtimeManager, "clubmembers");
		// TableManager tman = new TableManager();

		/*
		 * FIXME This does not work. Either create a runtime manager WITH a club
		 * or load a club from XML WITHOUT having a RuntimeManager.
		 */
		...
		final ClubMember dummy = new Athlete(333);
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					final Application app = new Application(runtimeManager);
